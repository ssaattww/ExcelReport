...
XSD スキーマ（`DslDefinition_v2.xsd`）と AST クラス群の対応を明示し、実装および保守に必要な情報をすべて含む。
...
- As-Is (Implemented): DSL namespace / schema は v2 のみサポート（`urn:excelreport:v2`, `DslDefinition_v2.xsd`）。v1 namespace は非互換で拒否。
...
- C# 式の評価:
  - `@( ... )` 形式の式評価
  - `use@with`、`sheet/repeat` の `from`（`@from` または `<from>`）、`Placement.when` の式評価
...
この章では `DslDefinition_v2.xsd` の主要要素と AST クラスの対応を一覧で示す。
...
| `<use component="" area="" with="">`   | `UseAst` + `Placement`                                    |
| `<repeat ... direction="">`（`@from/@var` または `<from>/<var>`） | `RepeatAst` + `Placement`                                 |
| `<sheet ...>`（`name`, `from/var` は属性または `<from>/<var>`） | `SheetAst.Name` / `FromExprRaw` / `VarName`              |
...
|-------------------------|----------------------------------------|
| `@name`                 | `SheetAst.Name`                        |
| `@from` / `<from>`      | `SheetAst.FromExprRaw`                 |
| `@var` / `<var>`        | `SheetAst.VarName`, `SheetAst.HasVarAttribute` |
...
`from` / `var` は属性と子要素の両記法を許容する。両方に指定された場合は `InvalidAttributeValue` の Warning を記録し、属性値を優先する。
...
| `@formulaRefScope`     | `CellAst.FormulaRefScope`           |
...
---
`value` は属性と子要素の両記法を許容する。両方に指定された場合は `InvalidAttributeValue` の Warning を記録し、属性値を優先する。
...
- `BuildSheetAst`:
  ...
  - `from` / `var` を属性または子要素（`<from>`/`<var>`）から解決する
  - `from` / `var` が属性と子要素の両方に存在する場合は Warning を記録し、属性値を優先する
...
3. **AST 構築後の DSL 検証フェーズ**
   ...
   - `SheetOptionsTargetNotFound`:
     - `FreezeAst.At`, `GroupRowsAst.At`, `GroupColsAst.At`, `AutoFilterAst.At` が sheet 内の `INamedAreaTarget.AreaName` として存在しない場合に発行
   - `FormulaRefSeriesNot1DContinuous`:
   ...
   - `InvalidAttributeValue`（Warning）:
     - `sheet` / `repeat` の `from` または `var` が属性と子要素の両方で指定された場合に発行（属性値を優先して継続）
     - `cell` の `value` が属性と子要素の両方で指定された場合に発行（属性値を優先して継続）
...
以下の属性値または要素値に `@( ... )` 形式の C# 式が現れる。

- `cell@value`
- `cell/<value>`
- `use@with`
- `sheet/repeat` の `from`（`@from` または `<from>`）
...
- `from` / `var` の属性・子要素競合
  - `sheet` / `repeat` で `@from` と `<from>`（または `@var` と `<var>`）を同時指定
  - `InvalidAttributeValue` の Warning が出ること
  - AST には属性値が採用されること
- `cell` の `value` 属性・子要素競合
  - `cell` で `@value` と `<value>` を同時指定
  - `InvalidAttributeValue` の Warning が出ること
  - AST には属性値が採用されること
