...
internal static class ExcelTemplateTestWorkbookFactory
{
    ...
    /// <summary>
    /// Creates a workbook fixture that defines sheet repeat in __sheet_meta/__workbook_meta shape.
    /// </summary>
    /// <returns>The temporary workbook path.</returns>
    public static string CreateWorkbookMetaSheetRepeatWorkbookFile()
    {
        const string workbookMetaXml =
            """
            <workbook>
              <sheets>
                <sheet templateSheet="InvoiceTemplate" name="@(grp.Name)" from="@(root.Groups)" var="grp" />
              </sheets>
            </workbook>
            """;
        ...
        var metaSheetPart = CreateWorksheetPart(workbookPart, sheetId: 1U, cells: []);
        var templateSheetPart = CreateWorksheetPart(
            workbookPart,
            sheetId: 2U,
            cells:
            [
                CreateInlineStringCell("A1", "@grp.Name"),
                CreateInlineStringCell("B1", "@grp.Total"),
            ]);
        var coverSheetPart = CreateWorksheetPart(
            workbookPart,
            sheetId: 3U,
            cells:
            [
                CreateInlineStringCell("A1", "Cover"),
            ]);
        ...
        sheets.Append(
            ...
            CreateSheet(coverSheetPart, workbookPart, "Cover", 3U));
        workbookPart.Workbook.Save();
