...
| FP131 | 10.8.9 は `GroupBlock` repeat を個別フレームではなく連続展開として扱い、1件目を `B4:D7`、2件目を `B8:D9`、親フレームを `A3:E10` とする図と説明へ修正する | 対応済み | 2026-04-12 |
| FP132 | 設計のぬけもれがないか、sub-agent（`gpt-5.3-codex` / `high`）でレビューし、指摘事項をレポート化する | 対応済み | 2026-04-12 |
...
| FP143 | issue #58 でヌケモレが見つかった場合は、実装前に phase/task へまとめて追加してから着手し、小出しで後追い追加しない。この指摘も必ずメモする | 対応中 | 2026-04-13 |
| FP144 | ユーザーが止めると言うか、当初目標が完了するまで自走し、指示待ちで止まらない | 対応中 | 2026-04-13 |
| FP145 | issue #58 は DSL互換を最優先にし、DSLへ変換できない実装には価値がない前提で進める | 対応中 | 2026-04-14 |
| FP146 | sub-agent レビュー結果は毎回 `reports/` にファイル出力してから共有する | 対応中 | 2026-04-14 |
| FP147 | issue番号を項目名に使う際は `#61` のみとせず、`ExcelTemplateのsheet repeat対応` のように内容が分かる名称にする | 対応中 | 2026-04-16 |
